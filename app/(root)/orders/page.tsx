...
export default function Orders({ searchParams }: { searchParams: { eventId?: string, query?: string } }) {
  ...
  const downloadPDF = () => {
    ...
    doc.text(`Total Revenue: ${formatPriceForPDF(totalRevenue/100)}`, 14, 40)
    ...
    doc.autoTable({
      ...
      body: orders.map(order => [
        ...
        formatPriceForPDF(order.totalAmount/100)
      ]),
      startY: 60,
    })
...
<div className="min-h-screen bg-gray-950 pt-[100px] lg:pt-[120px] text-gray-100">
      ...
      <Button 
        onClick={downloadPDF} 
        className="bg-blue-600 hover:bg-blue-700 text-white w-[25%] px-2 py-2 h-10"
